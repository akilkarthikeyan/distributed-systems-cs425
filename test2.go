	...
	// Prepare logs on each host: random content + controlled lines
	{
		...
		for i := 1; i <= len(hosts); i++ {
			...
			go func(host string, file string, idx int) {
				...
				// Change to home directory where server expects logs
				b.WriteString("cd \"$HOME\"; ")
				// base random block
				b.WriteString(fmt.Sprintf("head -c 65536 </dev/urandom | base64 > %s; ", file))
				// always present once (all hosts)
				b.WriteString(fmt.Sprintf("echo 'ALL %s' >> %s; ", tokAll, file))
				// present on first 3 hosts (some hosts)
				if idx < 3 {
					b.WriteString(fmt.Sprintf("for i in 1 2; do echo 'SOME %s' >> %s; done; ", tokSome, file))
				}
				// present only on host 0 (one host)
				if idx == 0 {
					b.WriteString(fmt.Sprintf("echo 'ONE %s' >> %s; ", tokOne, file))
				}
				// frequent on even-index hosts
				if idx%2 == 0 {
					b.WriteString(fmt.Sprintf("for i in $(seq 1 50); do echo 'FREQ %s' >> %s; done; ", tokFreq, file))
				} else {
					b.WriteString(fmt.Sprintf("for i in $(seq 1 5); do echo 'FREQ %s' >> %s; done; ", tokFreq, file))
				}
	...
	for _, tc := range tests {
		...
		for i, h := range hosts {
			...
			go func() {
				...
				// SSH ground truth
				out, sshErr := runSSH(h, fmt.Sprintf("cd \"$HOME\"; grep -nF -- '%s' machine.%d.log || true", tc.pattern, i+1))
				sshLines := []string{}
