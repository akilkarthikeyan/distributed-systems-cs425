...
// GetUUID generates a new UUID without hyphens for unique identifiers (nodes and chunks).
func GetUUID() string {
	return strings.ReplaceAll(uuid.New().String(), "-", "")
}

// KeyFor generates a unique membership key "IP:Port:Timestamp" that identifies a node instance.
func KeyFor(m Member) string { return fmt.Sprintf("%s:%d:%s", m.IP, m.Port, m.Timestamp) }

// GetRingId computes a 64-bit ring identifier from SHA-1 hash (first 8 bytes) for consistent hashing.
...
// GetRingSuccessor finds the immediate successor node on the consistent hash ring (wraps around, only Alive nodes).
...
// SnapshotMembers creates a snapshot of the membership list as a regular map (optionally excluding failed nodes).
...
// SelectKMembers randomly selects k members from the map (returns all if fewer than k available, for gossip protocol).
...
// EncodeFileToBase64 reads a file and encodes it to base64 for network transmission in JSON messages.
...
// DecodeBase64ToBytes decodes a base64 string to raw bytes for reconstructing file data from network.
...
// GetHyDFSCompliantFilename converts a filename to filesystem-safe by replacing slashes with underscores.
...
// MakeFileID hashes a filename (trimmed) to get a consistent 20-byte SHA-1 file ID.
...
// ShortID returns the low 16 bits of an ID as hex (for debugging only, never for ordering/placement).
...
// GetFileChunkPath returns the full path for a file chunk: hydfs/<compliant_filename>_<chunkID>
...
// compare160 compares two IDs as big-endian byte arrays (returns -1 if a<b, 0 if a==b, +1 if a>b).
func compare160(a, b ID) int {
	return bytes.Compare(a[:], b[:])
}

// RingTokenID computes a stable ring token SHA1(IP:port) that remains constant across restarts.
...
// Predecessor returns the index of the node immediately before idx, wrapping around (for primary interval).
...
// DebugString returns a readable dump of the ring order. It prints the full membership key (IP:port:timestamp) and a short hash tag.
...
// OwnersForFile returns n owners (primary + successors) walking clockwise from file position (allows same IP).
...
// OwnersForFileDistinctHosts returns up to n owners on distinct IPs (skips nodes sharing IPs for machine-level fault tolerance).
